...
cmds = [
    "cd /www/jmnav && git config user.email 'deploy@jmnav' && git config user.name 'deploy'",
    "cd /www/jmnav && rm -rf scripts/check_server.py scripts/deploy.py",
    ...
    "curl -s -o /dev/null -w 'HTTP_CODE:%{http_code}' http://127.0.0.1:41234/",
]
